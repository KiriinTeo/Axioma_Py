...
Para que você adicione uma função nova no sistema, comece pelo ***/core***, lá que fica a lógica do sistema e onde as coisas são tratadas, depois crie um service em ***/services*** em relação á nova função que desenvolveu e adicione ao facade (app_manager.py) e crie casos de uso sobre está função nova em ***/use_cases***, uma vez adicionado casos de uso, crie rotas em ***/api/routes***, use as demais rotas de exemplo em o que pedir para o usuário e afins, caso necessário padronização de dados adicione um novo esquema em ***/api/schemas*** ou caso queria manter persistência em banco precisa criar um novo modelo ORM em ***/infra/database/models*** use os demais de exemplo para aquilo cujo terá persistência em banco, caso também queira utilizar querys SQL com esse novo modelo ORM, adicione um repositório em ***/infra/database/repositories*** exemplos de como são um repostório estão na própria pasta ***/repositories*** com user_repo.py e dataset_repo.py. Dados exportados pelo sistema ficam presentes na pasta /data
